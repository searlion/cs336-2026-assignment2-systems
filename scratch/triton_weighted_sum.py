...
def weighted_sum_fwd(
    ...
    NUM_ROWS, D,                                              # D is the size of the last dimension of x -- the dimension your are summing over.
    ...
    # `tl.program_id` gives us a way to check which thread block we are running in
    row_tile_idx = tl.program_id(0)                          # tl.program_id(0) -- "Which worker am I?" -> 0, or 1, or 2, ...
    ...
    x_block_ptr = tl.make_block_ptr(                        # A block pointer is a moving window over a tensor.
        x_ptr,
        shape=(NUM_ROWS, D,),                               # true shape so it knows where the edges are
        strides=(x_stride_row, x_stride_dim),               # so it knows the memory layout
        offsets=(row_tile_idx * ROWS_TILE_SIZE, 0),         # for where the window starts. This is work splitting. Program 0 starts at row 0, program 1 starts at row 16...
        block_shape=(ROWS_TILE_SIZE, D_TILE_SIZE),          # for the window's size
        order=(1, 0),                                       # declares which axis is contiguous in memory -- last-dim-fastest, i.e. row-major, listed minor-to-major. A hint for the compiler's vectorization
    )

    weight_block_ptr = tl.make_block_ptr(                   # No program_id here. Every program reads the whole weight vector, because w is shared across all rows. Only x gets partitioned.
        weight_ptr,
        shape=(D,),
    ...
    output_block_ptr = tl.make_block_ptr(                   # One scalar per row. Same row_tile_idx * ROWS_TILE_SIZE offset as x, so this program writes exactly the outputs for the rows it reads.
        output_ptr,
        shape=(NUM_ROWS,),
...
def weighted_sum_backward(
    ...
    stride_xr, stride_xd,  # Grad outputs
    stride_wd,
    stride_gr,
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        # x.shape[:-1] returns a tuple of all dimensions except for the last one.
        D, output_dims = x.shape[-1], x.shape[:-1]

        # Reshape input tensor to 2D
        input_shape = x.shape
        x = rearrange(x, "... d -> (...) d")
        # Below stashes tensors so backward can get them later.
        # Looking at the math on page 20: computing ∂L/∂x needs weight, and ∂L/∂w needs x. So forward has to leave them somewhere.
        # Why not just use ctx.x = x? Ans: for correctness. save_for_backward records a version counter on each tensor. 
        # If something modifies x in place between the forward and the backward, PyTorch raises an error instead of silently
        # computing a gradient from the wrong values.
...
def weighted_sum(x, weight):
    ...
    print((weight * x).sum(axis=-1))
    return (weight * x).sum(axis=-1)

x = torch.randn(16,16).to("cuda")
w = torch.randn(16).to("cuda")

original = weighted_sum(x,w)
triton_version = f_weightedsum(x,w)
print(torch.testing.assert_close(original,triton_version))